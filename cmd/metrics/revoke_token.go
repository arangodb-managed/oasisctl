...
func init() {
	cmd.InitCommand(
		cmd.RevokeMetricsCmd,
		&cobra.Command{
			Use:   "token",
			Short: "Revoke a metrics token for a deployment",
		},
		func(c *cobra.Command, f *flag.FlagSet) {
			...
			c.Run = func(c *cobra.Command, args []string) {
				...
				// Revoke token
				if _, err := metricsc.RevokeToken(ctx, &common.IDOptions{Id: item.GetId()}); err != nil {
					log.Fatal().Err(err).Msg("Failed to revoke metrics token")
				}

				// Show result
				fmt.Println("Revoked metrics token!")
			}
		},
	)
}
