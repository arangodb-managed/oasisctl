...
func init() {
	cmd.InitCommand(
		cmd.DeleteMetricsCmd,
		&cobra.Command{
			Use:   "token",
			Short: "Delete a metrics token for a deployment",
		},
		func(c *cobra.Command, f *flag.FlagSet) {
			...
			c.Run = func(c *cobra.Command, args []string) {
				...
				// Delete token
				if _, err := metricsc.DeleteToken(ctx, &common.IDOptions{Id: item.GetId()}); err != nil {
					log.Fatal().Err(err).Msg("Failed to delete metrics token")
				}

				// Show result
				fmt.Println("Deleted metrics token!")
			}
		},
	)
}
