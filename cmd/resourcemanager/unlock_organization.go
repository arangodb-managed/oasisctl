...
func init() {
	cmd.InitCommand(
		...
		&cobra.Command{
			Use:   "organization",
			Short: "Unlock an organization, so it can be deleted",
		},
		func(c *cobra.Command, f *flag.FlagSet) {
			...
			c.Run = func(c *cobra.Command, args []string) {
				...
				// Fetch organzation
				item := selection.MustSelectOrganization(ctx, log, orgID, rmc)
				...
				if err != nil {
					log.Fatal().Err(err).Msg("Failed to unlock organzation")
				}

				// Show result
				fmt.Println("Unlocked organzation!")
