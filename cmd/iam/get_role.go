...
func getRoleCmdRun(c *cobra.Command, args []string) {
	// Validate arguments
	roleID, argsUsed := cmd.OptOption("role-id", getRoleArgs.roleID, args, 0)
	...
	// Fetch role
	item := selection.MustSelectRole(ctx, cmd.CLILog, roleID, getRoleArgs.organizationID, iamc, rmc)
