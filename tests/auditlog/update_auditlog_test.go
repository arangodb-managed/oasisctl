...
func TestUpdateAuditLog(t *testing.T) {
	...
	updatedName := "updated-test-auditlog"
	updateDescription := "updated-test-description"
	args := []string{"update", "auditlog", fmt.Sprintf("-i=%s", auditLog.GetId()), fmt.Sprintf("--name=%s", updatedName), fmt.Sprintf("--description=%s", updateDescription)}
	compare := `^Success!
Id           ` + auditLog.GetId() + `
Name         ` + updatedName + `
